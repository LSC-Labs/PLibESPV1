...
function getStylesheetTargetName(bCreateIfNotExist = true) {
    let strBasePath = path.join(getBasePath(false),"css");
    if(bCreateIfNotExist && !fs.existsSync(strBasePath)) fs.mkdirSync(strBasePath,{recursive: true})    
    return(
        path.join(
            strBasePath,
            Settings.getData("pageNames.stylesheetFile","_app.css")
        )
    );
...
// #region Stylesheet file joiner
function joinStylesheet(strPagePath) {
    let strInput = path.join(strPagePath,"page.css");
    if(fs.existsSync(strInput)) {
        let strOut = getStylesheetTargetName();
        console.log("   -> appending Stylesheet to : " + strOut);
        let strData = fs.readFileSync(strInput);
        fs.appendFileSync(strOut,strData);
        Status.numStyles++;
    }
...
function removeTargetPages() {
    ...
    if(fs.existsSync(getStylesheetTargetName())) fs.rmSync(getStylesheetTargetName());
}
...
function prepareAndWritePreamples() {
    ...
    writeTextToFile(getStylesheetTargetName(),CSS_PREAMBLE);
}
// #endregion
...
function processPages() {
    for(let strPageLocation of Settings.getData("usePages",[])) {
        ...
        joinStylesheet(strPageLocation);
